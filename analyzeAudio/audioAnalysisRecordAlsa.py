...
def recordAnalyzeAudio(midTermBufferSizeSec, modelName, modelType):
	'''
	...
	while 1:
		...
		if len(midTermBuffer) == midTermBufferSize:
			...
			if Classifier!=None:
				...
				if P[int(result)] < 0.4:
					confLevel = 1
				elif P[int(result)] >= 0.4 and P[int(result)]<0.6:
					confLevel = 2
				elif P[int(result)] >= 0.6 and P[int(result)] < 0.8:
					confLevel = 3
				else:
					confLevel = 4
				# print classNames[int(result)]
				print "<" + classNames[int(result)] + "><" + str(P[int(result)]) + "><" + str(time) + "><"+str(confLevel)+">"
			# allData = allData + midTermBuffer
...
def main(argv):
	...
	if argv[1] == '-recordAndClassifySegments':	# record input
		if (len(argv)==6):			# recording + audio analysis
			...
			recordAnalyzeAudio(2.0, modelName, modelType)
		else:
